...
impl QueryRoot {
    ...
    async fn getPost(
        ...
        match post {
            ...
            Err(err) => Err(match err {
                BlogError::NotFoundPost => FieldError::new("投稿が存在しません".to_string()),
                BlogError::ServerError(message) => FieldError::new(message.to_string()),
    ...
    async fn getPosts(
        ...
        let results = match posts {
            ...
            Err(err) => {
                return Err(match err {
                    BlogError::NotFoundPosts => FieldError::new("投稿がありません".to_string()),
                    BlogError::ServerError(message) => FieldError::new(message.to_string()),
                    _ => FieldError::new("unknown error".to_string()),
                })
            }
...
async fn pool() -> Result<MySqlPool, BlogError> {
    let url = match env::var("DATABASE_URL") {
        Ok(url) => url,
        Err(_) => {
            return Err(BlogError::ServerError(
...
pub async fn count() -> Result<i32, BlogError> {
    let pool = match pool().await {
        ...
        Err(_) => {
            return Err(BlogError::ServerError(
                "Database Error: connection failed".to_string(),
            ))
        }
    };
...
pub async fn get_posts(page: i32, category: String) -> Result<Vec<Post>, BlogError> {
    ...
    let posts = sqlx::query_as::<_, Post>(sql.as_str())
        .bind(offset)
        .fetch_all(&pool)
        .await;
